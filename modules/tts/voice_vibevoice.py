...
WORKFLOW_PATH = Path("modules/tts/vibevoice_api.json")
...
def tts_create_file(text: str, voice_file, voice_text) -> Path:
    ...
    if r.status_code != 200:
        raise RuntimeError(f"ComfyUI error {r.status_code}: {r.text}")
    prompt_id = r.json()["prompt_id"]
    start = time.time()
    while True:
        if time.time() - start > 300:
            raise TimeoutError("VibeVoice generation timeout")
        h = requests.get(f"{comfy_url}/history/{prompt_id}",timeout=60)
        ...
        if node and "audio" in node:
            ...
            return _download_audio(comfy_url, audio_info)
        time.sleep(0.3)
